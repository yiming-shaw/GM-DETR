...
def voc_get_image_info(annotation_root, im_id, a_path):
    ...
    filename = a_path.split('/')[-1].replace('.xml', '.jpg')
    assert filename is not None
    img_name = os.path.basename(filename)

    size = annotation_root.find('size')
    if size is None:
        img = cv2.imread(a_path.replace('Annotations', 'JPEGImages').replace('.xml', '.jpg'))
        width = img.shape[1]
        height = img.shape[0]
    else:
...
def Drone_voc_get_image_info(annotation_root, im_id, a_path):
    filename = a_path.split('/')[-1].replace('.xml', '.jpg')
    assert filename is not None
    img_name = os.path.basename(filename)
...
def Drone_voc_rotate_get_coco_annotation(obj, label2id):
    label_mapping = {
        'feright car': 'feright_car',
        'feright': 'feright_car',
        'truvk': 'truck',
        '*': 'others',  # 如果有无效的类名，可以将其映射为空字符串
    }
    ...
    if polygon is None:
        ...
        return anno
    x_coords = [float(polygon.findtext(f'x{i+1}')) for i in range(4)]
    y_coords = [float(polygon.findtext(f'y{i+1}')) for i in range(4)]
    assert len(x_coords) == len(y_coords) == 4, "Polygon coordinates error."
    xmin = min(x_coords)
    ymin = min(y_coords)
    xmax = max(x_coords)
    ymax = max(y_coords)
    ...
    anno = {
        ...
        'category_id': category_id,
        'ignore': 0,
        'segmentation': [[x_coords[0], y_coords[0], x_coords[1], y_coords[1], x_coords[2], y_coords[2], x_coords[3], y_coords[3]]]
    }
    return anno
...
def Drone_voc_xmls_to_cocojson(annotation_paths, label2id, output_dir, output_file):
    output_json_dict = {
        "images": [],
        "type": "instances",
        "annotations": [],
    ...
    for a_path in tqdm(annotation_paths):
        ...
        img_info = Drone_voc_get_image_info(ann_root, im_id, a_path)
        output_json_dict['images'].append(img_info)
        for obj in ann_root.findall('object'):
            ann = Drone_voc_rotate_get_coco_annotation(obj=obj, label2id=label2id)
            ann.update({'image_id': im_id, 'id': bnd_id})
            output_json_dict['annotations'].append(ann)
            bnd_id = bnd_id + 1
        im_id += 1
...
def main():
    ...
    try:
        assert args.dataset_type in ['voc', 'Drone_voc', 'labelme', 'cityscape', 'widerface']
    ...
    elif args.dataset_type == 'Drone_voc':
        assert args.voc_anno_dir and args.voc_anno_list and args.voc_label_list
        label2id, ann_paths = voc_get_label_anno(
            args.voc_anno_dir, args.voc_anno_list, args.voc_label_list)
        Drone_voc_xmls_to_cocojson(
            annotation_paths=ann_paths,
            label2id=label2id,
            output_dir=args.output_dir,
            output_file=args.voc_out_name)
