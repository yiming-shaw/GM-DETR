...
__all__ = ['PPHGNetV2X6', 'PPHGNetV2X6deploy']
...
@register
@serializable
class PPHGNetV2X6deploy(nn.Layer):
    """
    PPHGNetV2
    Args:
        stem_channels: list. Number of channels for the stem block.
    ...
    def forward(self, inputs):
        # x = inputs['image']
        # x_1 = inputs['image_1']
        x, x_1 = paddle.split(inputs['image'], num_or_sections=[3, 3], axis=1)
        x = self.stem(x)
        x_1 = self.stem(x_1)
        outs = []
        for idx, stage in enumerate(self.stages):
